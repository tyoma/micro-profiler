...
namespace micro_profiler
{
		...
		void *interceptor, hooks<void>::on_enter_t *on_enter, hooks<void>::on_exit_t *on_exit)
	{
		...
		_fragment_length = calculate_fragment_length(body, jmp_size);

		byte_range source_chunk(_target_function, _fragment_length);
		const size_t size0 = c_thunk_size + _fragment_length + jmp_size;
		...
		reinterpret_cast<intel::jmp_rel_imm32 *>(thunk + c_thunk_size + _fragment_length)
			->init(_target_function + _fragment_length);
		...
		mem_set(_target_function + jmp_size, 0xCC, _fragment_length - jmp_size);
	}

	function_patch::~function_patch()
	{
		scoped_unprotect su(byte_range(_target_function, _fragment_length));

		mem_copy(_target_function, _saved, _fragment_length);
	}
}
