	...
	namespace tests
	{
		...
		begin_test_suite( StatisticsBridgeTests )
			...
			test( LoadedModulesAreReportedOnUpdate )
			{
				...
				assert_not_equal(string::npos, _state.update_log[0].image_loads[0].second.find("SYMBOL_CONTAINER_1.DL"));
				...
				assert_not_equal(string::npos, _state.update_log[1].image_loads[0].second.find("SYMBOL_CONTAINER_2.DL"));
				assert_equal(reinterpret_cast<uintptr_t>(_images.at(2).load_address()),
					_state.update_log[1].image_loads[1].first);
				assert_not_equal(string::npos, _state.update_log[1].image_loads[1].second.find("SYMBOL_CONTAINER_3_NOSYMBOLS.DL"));
			}
