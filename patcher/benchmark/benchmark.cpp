...
namespace micro_profiler
{
	...
	float measureHookOverhead(unsigned repetitions)
	{
		stopwatch sw;
...
int main()
{
	...
	printf("Hooked call time (minimal): %.1fns\n", measureHookOverhead<minimal_interceptor>(c_repetitions));
	printf("Hooked call time (flat queue): %.1fns\n", measureHookOverhead<queue_interceptor<single_queue_manager<flat_queue>>>(c_repetitions));
	printf("Hooked call time (VLE queue): %.1fns\n", measureHookOverhead<queue_interceptor<single_queue_manager<vle_queue>>>(c_repetitions));
	printf("Hooked call time (tls, flat queue): %.1fns\n", measureHookOverhead<queue_interceptor<tls_queue_manager<flat_queue>>>(c_repetitions));
	printf("Hooked call time (tls, VLE queue): %.1fns\n", measureHookOverhead<queue_interceptor<tls_queue_manager<vle_queue>>>(c_repetitions));
	return 0;
}
