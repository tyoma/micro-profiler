...
namespace micro_profiler
{
	namespace tests
	{
		...
		public:
			...
			[TestMethod]
			void EscapeNumbersUnderPrecisionAsZeroes()
			{
				// INIT / ACT / ASSERT
				Assert::IsTrue(L"0s" == format_interval_proxy(0.00000000049));
				Assert::IsTrue(L"0s" == format_interval_proxy(-0.00000000049));
				Assert::IsTrue(L"0s" == format_interval_proxy(-0.00000000000049));
				Assert::IsTrue(L"0s" == format_interval_proxy(numeric_limits<double>::min()));
			}
			...
			{
				// INIT / ACT / ASSERT
				Assert::IsTrue(L"1ms" == format_interval_proxy(0.0009995000000000001));
			...
			{
				// INIT / ACT / ASSERT
				Assert::IsTrue(L"3\x03bcs" == format_interval_proxy(0.000003));
				Assert::IsTrue(L"2.09\x03bcs" == format_interval_proxy(0.0000020949));
				Assert::IsTrue(L"1.97\x03bcs" == format_interval_proxy(0.00000197));
				Assert::IsTrue(L"13.6\x03bcs" == format_interval_proxy(0.00001364555));
				Assert::IsTrue(L"117\x03bcs" == format_interval_proxy(0.000117489));
				Assert::IsTrue(L"999\x03bcs" == format_interval_proxy(0.00099945555));
				Assert::IsTrue(L"-7.91\x03bcs" == format_interval_proxy(-0.000007914));
				Assert::IsTrue(L"-5.09\x03bcs" == format_interval_proxy(-0.000005094));
				Assert::IsTrue(L"-17\x03bcs" == format_interval_proxy(-0.000017));
				Assert::IsTrue(L"-231\x03bcs" == format_interval_proxy(-0.000231));
				Assert::IsTrue(L"-999\x03bcs" == format_interval_proxy(-0.000999));
			}
