// Taken from: https://github.com/Nomade040/length-disassembler
...
inline void parseModRM(uint8_t** b, const bool addressPrefix)
{
	...
	if (!addressPrefix || (addressPrefix && **b >= 0x40))
	{
		...
		if (**b < 0xC0 && (**b & 7 /*0b111*/) == 4 /*0b100 */&& !addressPrefix)
			hasSIB = true, (*b)++;

		if (modrm >= 0x40 && modrm <= 0x7F) // disp8 (ModR/M)
			(*b)++;
		else if ((modrm <= 0x3F && (modrm & 7 /*0b111*/) == 5 /*0b101*/) || (modrm >= 0x80 && modrm <= 0xBF)) //disp16,32 (ModR/M)
			*b += (addressPrefix) ? 2 : 4;
		else if (hasSIB && (**b & 7 /*0b111*/) == 5 /*0b101*/) //disp8,32 (SIB)
			*b += (modrm & 64 /*0b01000000*/) ? 1 : 4;
	}
