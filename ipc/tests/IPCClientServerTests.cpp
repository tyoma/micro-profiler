		...
		namespace named_pipes
		{
			namespace tests
			{
				namespace
				{
					...
					class mock_session : public ipc::named_pipes::server::session, noncopyable
					{
					public:
					...
					class server : public ipc::named_pipes::server
					{
					public:
						server(const char *name)
							: ipc::named_pipes::server(name), data(new server_data), _thread(bind(&server::run, this))
						{	}
